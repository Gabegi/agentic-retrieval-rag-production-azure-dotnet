...
{
    private static ChunkStatsAdapter Document(string id) => new() { Id = id, Content = "content" };
        ...
        Exception? statsException = null)
    {
        ...
        mock.Setup(m => m.UpsertDocumentsAsync(It.IsAny<IEnumerable<ChunkStatsAdapter>>(), It.IsAny<CancellationToken>()))
    ...
    private static CsvUploadService BuildService(
        Mock<IIndexDocumentService> indexDocumentService, Mock<IIndexStatsMonitor>? indexStatsMonitor = null) =>
        new(indexDocumentService.Object, (indexStatsMonitor ?? MockIndexStatsMonitor()).Object, NullLogger<CsvUploadService>.Instance);
