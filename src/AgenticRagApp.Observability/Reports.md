...
| `pdf-file-facts` | `.../{ts}-pdf-file-facts-{instanceId}.json` | `ExtractionReporter` | Per-document facts, one row per extracted document: title, SHA-256 content hash of the raw bytes, page/heading/table/figure counts, content length. The PdfPig-era columns (file size, spec version, native Producer/Creator/Subject/Keywords) went with the preflight. The content hash is the durable half of the duplicate/cache evidence — the run log only says how many were distinct |
