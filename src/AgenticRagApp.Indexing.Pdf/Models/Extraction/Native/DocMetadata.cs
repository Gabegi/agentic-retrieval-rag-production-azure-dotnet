...
public record DocMetadata(
    ...
    // PdfPig's own per-page size (MediaBox), in points - Unit is always "point" here,
    // never "inch" like GetPageDimensionsHelper's DI-side PageDimensions. Compared
    // against DI's reported dimensions by PageDimensionWarningsHelper. Null means the
    // PdfPig read itself failed (see TryGetPageDimensions); never empty for a valid PDF.
    IReadOnlyList<PageDimensions>? NativePageDimensions);
