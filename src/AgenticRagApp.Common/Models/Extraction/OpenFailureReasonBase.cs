...
// can't share a base type, and each source has its own closed set of failure categories
// that make sense for it (PdfOpenFailureReason today; the archived CSV pipeline had its
// own) - so each source defines its own sealed record of static instances deriving from
// this, instead of one shared enum neither source's categories cleanly fit.
//
