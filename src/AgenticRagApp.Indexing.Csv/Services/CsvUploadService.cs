...
// Kept separate from CsvEmbeddingService so the two concerns can evolve independently.
public class CsvUploadService : ICsvUploadService
{
    ...
    public async Task<CsvUploadResult> UploadDocumentsAsync(
        IEnumerable<AgenticRagApp.Common.Models.ChunkStatsAdapter> documents, IReadOnlyList<string> staleDocumentIds, CancellationToken ct = default)
    {
