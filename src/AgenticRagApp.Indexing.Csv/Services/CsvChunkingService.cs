...
public class CsvChunkingService : ICsvChunkingService
{
    private readonly ICsvChunkingStrategy             _strategy;
    private readonly ILogger<CsvChunkingService>      _logger;

    public string Name => _strategy.Name;

    public CsvChunkingService(ICsvChunkingStrategy strategy, ILogger<CsvChunkingService> logger)
    {
        _strategy = strategy;
        _logger   = logger;
    }
    ...
    public (IReadOnlyList<ChunkStatsAdapter> Docs, ChunkingStageMetrics Stats) ChunkDocuments(
        IReadOnlyList<ExtractionDocument> docs)
    {
        var result = new List<ChunkStatsAdapter>();
        ...
        foreach (var doc in docs.OrderBy(d => d.SourceId).ThenBy(d => d.Ordinal))
        {
            ...
            for (int docChunkIndex = 0; docChunkIndex < chunks.Count; docChunkIndex++)
            {
                ...
                result.Add(new ChunkStatsAdapter
                {
                    Id               = CsvChunkingUtils.SafeKey($"{doc.SourceId}::{doc.Ordinal}", docChunkIndex),
    ...
    private static void EmitChunkMetrics(ChunkingStageMetrics stats, IReadOnlyList<ChunkStatsAdapter> chunks)
    {
