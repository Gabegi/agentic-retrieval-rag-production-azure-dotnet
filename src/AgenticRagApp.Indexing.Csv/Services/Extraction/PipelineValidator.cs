...
public class PipelineValidator : IPipelineValidator
{
    ...
    // 1. Aggregate every error/warning bucket into one place.
            ...
            //   + cleanResult.Warnings                      (Warning, Stage=Clean)
            //   + joinResult.SkippedIndexRecords            (Warning, Stage=Join — "no pages" docs)
    private static List<PipelineIssue> CollectIssues(
        ExtractionBatch<PageRecord>  pagesExtraction,
        ExtractionBatch<IndexRecord> indexExtraction,
        JoinResult                    joinResult,
        CleanResult                   cleanResult,
        List<string>                  redFlags)
    {
        ...
        // CsvExtractor is generic over both CSVs, so it can only stamp ParsePages. The
        // pages/index distinction is known here and nowhere else, so it is re-stamped -
        // this is the one place the caller legitimately knows the stage better than the
        // producer. Cleaning issues already carry their own (Clean/TextQuality) and are
        // passed through untouched.
        //     pagesExtraction.Errors                    (Error,   Stage=Parse:Pages)
        issues.AddRange(pagesExtraction.Errors.Select(e => e with { Stage = PipelineStage.ParsePages }));

        //   + indexExtraction.Errors                    (Error,   Stage=Parse:Index)
        issues.AddRange(indexExtraction.Errors.Select(e => e with { Stage = PipelineStage.ParseIndex }));

        //   + joinResult.Errors                         (Error,   Stage=Join)
        issues.AddRange(joinResult.Errors.Select(e =>
            PipelineIssue.Error(PipelineStage.Join, e.DocumentId, e.Message)));

        //   + joinResult.DataQualityWarnings            (Warning, Stage=Join)
        issues.AddRange(joinResult.DataQualityWarnings.Select(w =>
            PipelineIssue.Warning(PipelineStage.Join, w.DocumentId, w.Message)));

        //   + cleanResult issues                        (Clean / TextQuality, own severity)
        issues.AddRange(cleanResult.Issues);

        // 1b. Index documents with no pages never reach the search index — make that visible

        //   + joinResult.SkippedIndexRecords            (Warning, Stage=Join — "no pages" docs)
        issues.AddRange(joinResult.SkippedIndexRecords.Select(r => PipelineIssue.Warning(
            PipelineStage.Join,
            r.DocumentId,
            "Index record has no pages — document will not be indexed.")));
    ...
    private static List<PipelineIssue> TextNTableQualityCheck(CleanResult cleanResult)
    {
        var issues = new List<PipelineIssue>();
        ...
        foreach (var record in cleanResult.Records)
        {
            ...
            if (replacementCount > 0)
                issues.Add(PipelineIssue.Error(PipelineStage.TextQuality,
                    record.DocumentId,
                    $"Page {record.PageIndex}: {replacementCount} U+FFFD char(s) — source text is corrupted."));
            ...
            if (corruptCharCount > 0)
                issues.Add(PipelineIssue.Error(PipelineStage.TextQuality,
                    record.DocumentId,
                    $"Page {record.PageIndex}: {corruptCharCount} control/unassigned character(s) — likely encoding corruption."));
            ...
            if (!string.IsNullOrEmpty(record.Language) &&
                ...
                issues.Add(PipelineIssue.Warning(PipelineStage.TextQuality,
                    record.DocumentId,
                    $"Page {record.PageIndex}: language '{record.Language}' — nl.microsoft analyzer will tokenize this poorly."));
            ...
            foreach (var block in tableBlocks)
            {
                ...
                if (pipeCounts.Count > 1 && pipeCounts.Distinct().Count() > 1)
                {
                    issues.Add(PipelineIssue.Warning(PipelineStage.TextQuality,
                        record.DocumentId,
                        $"Page {record.PageIndex}: markdown table has inconsistent column counts across rows."));
