...
public class DataCleaner : IDataCleaner
{
    ...
    private static void ReportDuplicatePage(JoinedPageRecord page, CleanResult result)
    {
        ...
        result.AddIssue(PipelineIssue.Warning(
            PipelineStage.Clean,
            page.DocumentId,
            $"Duplicate page {page.PageIndex} in source — kept the first occurrence."));
    }
        ...
        try
        {
            ...
            if (mojibakeFixed)
            {
                ...
                result.AddIssue(PipelineIssue.Warning(
                    ...
                    page.DocumentId,
                    $"Page {page.PageIndex}: repaired mojibake in source text (e.g. 'â€™' -> \"'\")."));
            }
            ...
            if (string.IsNullOrWhiteSpace(content))
                result.AddIssue(PipelineIssue.Warning(
                    PipelineStage.Clean,
                    page.DocumentId,
                    $"PageContent is empty after cleanup (page {page.PageIndex}) — likely a blank source page."));

            result.AddRecord(ToCleanedRecord(page, content));
        }
        catch (Exception ex)
        {
            result.AddIssue(PipelineIssue.Error(PipelineStage.Clean, page.DocumentId, ex.Message));
        }
    }
