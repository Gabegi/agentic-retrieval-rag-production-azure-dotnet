...
internal static class DocumentProfileHelper
{
    ...
    public static DocumentProfile Compute(
        // Each page's CLEANED text, in page order - the same strings the document's Content is
        // assembled from, which is what these thresholds were calibrated against. Took
        // PdfPageRecord until that model died with the Document Intelligence pipeline; the
        // record carried a blob name, page number and picture-only flag that this computation
        // never read, so plain text is both smaller and the honest input.
        ...
        // Length of the service's own RAW markdown. Every Heading.Offset addresses THAT string,
        // so it is the only end bracket B5 can be measured against - see MaxGap. Optional so a
        // caller with no raw content in hand gets an under-report rather than a mixed-coordinate
        // number.
        int? rawContentLength = null)
    {
        ...
        // have already caught (a response with zero pages fails validation upstream - see
        // ContentUnderstandingAnalyzer.ValidateAnalyzeResult) - guarded here anyway rather
        ...
        {
            foreach (var (isTable, text) in ChunkingHelper.SplitIntoBlocks(page))
            {
        ...
        var numberedHeadingShare = headings.Count == 0
            ? 0
            : headings.Count(h => HeadingNumbering.NumberedHeadingPrefix().IsMatch(h.Content.Trim())) / (double)headings.Count;
