...
{
    private static Mock<ICsvChunkingStrategy> MockStrategy(string name = "TestStrategy", Func<string, IReadOnlyList<TextChunk>>? chunkFn = null)
    {
        var mock = new Mock<ICsvChunkingStrategy>();
    ...
    private static CsvChunkingService BuildService(Mock<ICsvChunkingStrategy> strategy) =>
        new(strategy.Object, NullLogger<CsvChunkingService>.Instance);
    ...
    public void ChunkDocuments_ComputesIdFromSourceIdOrdinalAndChunkIndex()
    {
        ...
        var expectedId = CsvChunkingUtils.SafeKey("doc1::2", 0);
