...
public class ExtractionServiceTests
{
    ...
    private static Mock<ICsvExtractionOrchestrator> MockExtractor(ExtractionOutput output, string source = "csv")
    {
        var mock = new Mock<ICsvExtractionOrchestrator>();
    ...
    private static CsvExtractionService BuildService(
        Mock<ICsvExtractionOrchestrator> extractor, Mock<IIndexDocumentService> indexService, Mock<IRunReportWriter> reportWriter) =>
        new(extractor.Object, indexService.Object, reportWriter.Object, NullLogger<CsvExtractionService>.Instance);
